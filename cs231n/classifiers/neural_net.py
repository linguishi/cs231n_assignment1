...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    L1_out = X.dot(W1) + b1.reshape(1, -1) # shape N * H
    L1_Relu_out = np.maximum(0, L1_out)
    scores = L1_Relu_out.dot(W2) + b2.reshape(1, -1) # shape N * C
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    score_reducted = scores - score_max.reshape(-1, 1)
    score_non_log = np.exp(score_reducted)
    loss_data = score_max - scores[np.arange(N), y] + np.log(np.sum(score_non_log, axis=1))
    loss = np.sum(loss_data)/N + 0.5 * reg * (np.sum(W1 * W1) + np.sum(W2 * W2))
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    # combine W2 and b2 to make parameter matrix Wb2, to compute Wb2, need to 
    # construct X2
